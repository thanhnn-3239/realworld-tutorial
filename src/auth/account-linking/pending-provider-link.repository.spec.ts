...
describe('PendingProviderLinkRepository', () => {
  ...
  describe('issue', () => {
    ...
    it('returns cooldown without rotating when within 60 seconds', async () => {
      ...
      expect(mockPrisma.pendingAuthProviderLink.update).not.toHaveBeenCalled();
    ...
    it('rotates token when past 60 seconds cooldown', async () => {
      ...
      const rotatedRow = fakePendingRow({
        id: 42,
      ...
      mockPrisma.pendingAuthProviderLink.update.mockResolvedValue(rotatedRow);

      const result = await repository.issue(
        fakeIssueInput({ tokenHash: 'rotated-hash', now }),
      );
      ...
      expect(result).toEqual({
        ...
        pendingId: 42,
        tokenHash: 'rotated-hash',
        expiresAt: rotatedRow.expiresAt,
      });
      expect(mockPrisma.pendingAuthProviderLink.update).toHaveBeenCalledWith({
        where: { id: 42 },
